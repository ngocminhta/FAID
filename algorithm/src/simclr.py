...
class SimCLR_Classifier_SCL(nn.Module):
    ...
    def _compute_logits(self, q, q_index1, q_index2, q_label, k, k_index1, k_index2, k_label):
        ...
        # model:model set
        pos_logits_model = torch.sum(logits * same_label, dim=1) / torch.max(
            torch.sum(same_label, dim=1), self.esp
        )
    ...
    def forward(self, batch, indices1, indices2, label):
        ...
        # q:N
        # k:4N
        logits_label = self._compute_logits(
        ...
        loss = self.a * loss_label + self.c * loss_classfiy
        if self.training:
...
class SimCLR_Classifier(nn.Module):
    ...
    def _compute_logits(self, q, q_index1, q_index2, q_label, k, k_index1, k_index2, k_label):
        ...
        # human: human
        pos_logits_human = torch.sum(logits * same_label, dim=1) / torch.max(
            torch.sum(same_label, dim=1), self.esp
        )
        ...
        # human+ai: general
        pos_logits_mixed = torch.sum(logits * same_mixed, dim=1) / torch.maximum(
            torch.sum(same_mixed, dim=1), self.esp
        )
        ...
        # human+ai: model
        pos_logits_mixed_set = torch.sum(
            logits * torch.logical_and(same_mixed, same_set), dim=1
        ) / torch.max(torch.sum(torch.logical_and(same_mixed, same_set), dim=1), self.esp)
        neg_logits_mixed_set = logits * torch.logical_not(torch.logical_and(same_mixed, same_set))
        logits_mixed_set = torch.cat(
            (pos_logits_mixed_set.unsqueeze(1), neg_logits_mixed_set), dim=1
        )
        logits_mixed_set = logits_mixed_set[is_mixed]

        # model set:label
        pos_logits_set = torch.sum(logits * same_set, dim=1) / torch.max(
            torch.sum(same_set, dim=1), self.esp
        )
        ...
        # label: label
        pos_logits_label = torch.sum(logits * same_label, dim=1) / torch.max(
            torch.sum(same_label, dim=1), self.esp
        )
    ...
    def forward(self, encoded_batch, label, indices1, indices2):  # , weights):
        ...
        # q:N
        # k:4N
        logits_human, logits_mixed, logits_mixed_set, logits_set, logits_label = (
            self._compute_logits(q, indices1, indices2, label, k, k_index1, k_index2, k_label)
        )
        out = self.classifier(q)
        ...
        gt_mixed = torch.zeros(logits_mixed.size(0), dtype=torch.long, device=logits_mixed.device)
        gt_mixed_set = torch.zeros(
            logits_mixed_set.size(0), dtype=torch.long, device=logits_mixed_set.device
        )
        ...
        loss_mixed = F.cross_entropy(logits_mixed, gt_mixed)
        loss_mixed_set = F.cross_entropy(logits_mixed_set, gt_mixed_set)
        ...
        loss = (
            self.a * loss_set
            + (4 * self.b - self.a) * loss_label
            + self.b * loss_human
            + self.b * loss_mixed
            + 2 * self.b * loss_mixed_set
            + self.c * loss_classfiy
        )
