...
from utils.load_dataset import TextDataset, load_dataset, load_dataset_conditional_lang
from utils.utils import calculate_metrics, compute_metrics_train
...
def train(opt):
    ...
    if (
        opt.dataset == "faidset"
        or opt.dataset == "llmdetectaive"
        or opt.dataset == "hart"
        or opt.dataset == "faidsetv2"
    ):
    ...
    for epoch in range(opt.total_epoch):
        ...
        for i, batch in pbar:
            ...
            if opt.one_loss:
                loss, loss_label, loss_classfiy, k_out, k_outlabel = model(
                    encoded_batch, label, is_mixed, write_model
                )  # , weights)
            else:
                (
                    loss,
                    ...
                    k_out,
                    k_outlabel,
                ) = model(encoded_batch, label, is_mixed, write_model)  # , weights)
        ...
        if opt.only_classifier == False and fabric.global_rank == 0:
            ...
            if opt.AA:
                ...
                human_rec, machine_rec, avg_rec, acc, precision, recall, f1 = compute_metrics_train(
                    test_labels, preds
                )
                print(
                    f"Validation HumanRec: {human_rec}, MachineRec: {machine_rec}, AvgRec: {avg_rec}, Acc:{acc}, Precision:{precision}, Recall:{recall}, F1:{f1}"
                )
                writer.add_scalar("val/val_loss", test_loss, epoch)
        ...
        if fabric.global_rank == 0:
            ...
            torch.save(
                model.get_encoder().state_dict(), os.path.join(opt.savedir, "model_last.pth")
            )
            torch.save(model.state_dict(), os.path.join(opt.savedir, "model_classifier_last.pth"))
            print(
                "Save model to {}".format(os.path.join(opt.savedir, "model_last.pth")), flush=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--projection_size", type=int, default=768, help="Pretrained model output dim"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--per_gpu_batch_size", default=32, type=int, help="Batch size per GPU for training."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dataset", type=str, default="faidsetv2", help="faidset, llmdetectaive, hart, faidsetv2"
    )
    # path to retrieve dataset
    parser.add_argument("--path", type=str, default="data/FAIDSetv2")
    parser.add_argument(
        "--database_name", type=str, default="train", help="train,valid,test,test_ood"
    )
    parser.add_argument(
        "--test_dataset_name", type=str, default="test", help="train,valid,test,test_ood"
    )
    parser.add_argument(
        "--topk", type=int, default=50, help="Search topk nearest neighbors for validation"
    )

    parser.add_argument("--a", type=float, default=2)
    parser.add_argument("--b", type=float, default=1)
    parser.add_argument("--c", type=float, default=1, help="classifier loss weight")
    ...
    parser.add_argument(
        "--total_epoch", type=int, default=100, help="Total number of training epochs"
    )
    ...
    parser.add_argument("--name", type=str, default="faidsetv2")
    ...
    parser.add_argument(
        "--freeze_embedding_layer", action="store_true", help="freeze embedding layer"
    )
    parser.add_argument("--one_loss", action="store_true", help="only use single contrastive loss")
    parser.add_argument(
        "--only_classifier", action="store_true", help="only use classifier, no contrastive loss"
    )
